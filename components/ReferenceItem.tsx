...
const ReferenceItem: React.FC<ReferenceItemProps> = React.memo(({ 
  ...
  if (viewMode === 'list') {
    return (
      <div
        {...commonDivProps}
        className={`group relative w-full rounded-lg transition-all duration-150 ease-in-out flex items-center p-2 space-x-3 bg-white border border-gray-200 hover:shadow-sm ${stateClasses}`}
      >
        {previewImageUrl ? (
          <img 
            src={previewImageUrl} 
            alt={`Preview for ${reference.text}`} 
            className="w-14 h-14 object-cover rounded-md flex-shrink-0 pointer-events-none"
          />
        ) : (
          <div className="w-14 h-14 bg-gray-100 rounded-md flex items-center justify-center flex-shrink-0 border border-gray-200 pointer-events-none">
        ...
        <button 
          ...
          className="ml-2 p-1 text-gray-400 hover:text-red-600 opacity-0 group-hover:opacity-100 transition-opacity focus:opacity-100 focus:ring-1 focus:ring-red-300 rounded-full hover:bg-red-50 flex-shrink-0"
  ...
  // Grid View (default)
  let baseClasses = "rounded-lg transition-all duration-150 ease-in-out flex flex-col shadow-sm bg-white";
  return (
    <div
      {...commonDivProps}
    ...
    >
      {previewImageUrl ? (
        ...
        <div className="w-full h-32 bg-gray-100 rounded-t-lg flex items-center justify-center border-b border-gray-200 pointer-events-none">
          <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-12 h-12 text-gray-400">
            <path strokeLinecap="round" strokeLinejoin="round" d="M2.25 15.75l5.159-5.159a2.25 2.25 0 013.182 0l5.159 5.159m-1.5-1.5l1.409-1.409a2.25 2.25 0 013.182 0l2.909 2.909m-18 3.75h16.5a1.5 1.5 0 001.5-1.5V6a1.5 1.5 0 00-1.5-1.5H3.75A1.5 1.5 0 002.25 6v12a1.5 1.5 0 001.5 1.5zm10.5-11.25h.008v.008h-.008V8.25zm.375 0a.375.375 0 11-.75 0 .375.375 0 01.75 0z" />
          </svg>
        </div>
      )}
      <div className="p-3 flex justify-between items-center rounded-b-lg">
